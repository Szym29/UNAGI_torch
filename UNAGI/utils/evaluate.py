...
def run_metrics(adatas, cell_type_key, stage_key):
    ...
    for i,stage in enumerate(stage_keys):
        ...
        isolated_asw = scib.metrics.isolated_labels_asw(adata,label_key = 'UNAGI', batch_key = 'stage', embed='z')
        celltype_asw = silhouette_score(adata.obsm['z'], adata.obs['UNAGI'])
        isolated_f1 = scib.metrics.isolated_labels_f1(adata,label_key = 'UNAGI', batch_key = 'stage', embed='z')
        print('ARI: ', ari)
        print('NMIs: ', nmi)
